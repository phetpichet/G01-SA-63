...
func (fq *FormQuery) QueryDrugs() *DrugQuery {
	query := &DrugQuery{config: fq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(form.Table, form.FieldID, fq.sqlQuery()),
...
// First returns the first Form entity in the query. Returns *NotFoundError when no form was found.
...
// FirstID returns the first Form id in the query. Returns *NotFoundError when no id was found.
...
func (fq *FormQuery) FirstXID(ctx context.Context) int {
...
// Only returns the only Form entity in the query, returns an error if not exactly one entity was returned.
...
// OnlyID returns the only Form id in the query, returns an error if not exactly one id was returned.
...
func (fq *FormQuery) WithDrugs(opts ...func(*DrugQuery)) *FormQuery {
	query := &DrugQuery{config: fq.config}
...
func (fq *FormQuery) sqlAll(ctx context.Context) ([]*Form, error) {
	...
	if query := fq.withDrugs; query != nil {
		...
		query.Where(predicate.Drug(func(s *sql.Selector) {
			s.Where(sql.InValues(form.DrugsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.form_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "form_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "form_id" returned %v for node %v`, *fk, n.ID)
			}
...
func (fq *FormQuery) sqlQuery() *sql.Selector {
...
// FormGroupBy is the builder for group-by Form entities.
...
// FormSelect is the builder for select fields of Form entities.
