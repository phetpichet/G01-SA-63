...
func (uq *UnitQuery) QueryDrugs() *DrugQuery {
	query := &DrugQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(unit.Table, unit.FieldID, uq.sqlQuery()),
...
// First returns the first Unit entity in the query. Returns *NotFoundError when no unit was found.
...
// FirstID returns the first Unit id in the query. Returns *NotFoundError when no id was found.
...
func (uq *UnitQuery) FirstXID(ctx context.Context) int {
...
// Only returns the only Unit entity in the query, returns an error if not exactly one entity was returned.
...
// OnlyID returns the only Unit id in the query, returns an error if not exactly one id was returned.
...
func (uq *UnitQuery) WithDrugs(opts ...func(*DrugQuery)) *UnitQuery {
	query := &DrugQuery{config: uq.config}
...
func (uq *UnitQuery) sqlAll(ctx context.Context) ([]*Unit, error) {
	...
	if query := uq.withDrugs; query != nil {
		...
		query.Where(predicate.Drug(func(s *sql.Selector) {
			s.Where(sql.InValues(unit.DrugsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.unit_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "unit_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "unit_id" returned %v for node %v`, *fk, n.ID)
			}
...
func (uq *UnitQuery) sqlQuery() *sql.Selector {
...
// UnitGroupBy is the builder for group-by Unit entities.
...
// UnitSelect is the builder for select fields of Unit entities.
